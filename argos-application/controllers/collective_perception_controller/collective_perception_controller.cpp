...
std::queue<SEventData> CCollectivePerception::RecordEvents() 
{
   ...
   for(size_t i = 0; i < tBlobReads.BlobList.size(); ++i) {
      ...
      if (bLocalLeader) {
         ...
         sEvent.Location = SLocation(cPos.GetX(), cPos.GetY());
         sEvents.push(sEvent);
      }
   }
   return sEvents;
...
CVector2 CCollectivePerception::ComputeAbsolutePosition(const CVector2& c_coordEvent)
{
